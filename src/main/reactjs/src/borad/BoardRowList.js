...
import aru from '../image/aru.gif';

function BoardRowList(props) {
    const {idx,no,row,currentPage}=props;
    const smallurl1=process.env.REACT_APP_BOARDSMALLURL;
    const smallurl2=process.env.REACT_APP_BOARDSMALLURL2;
        ...
        <tr>
            ...
            <td>
                <NavLink to={`/board/detail/${row.num}/${currentPage}`}
                    ...
                    {/* 40x40 썸네일 이미지 나오게 하기(사진이 없을경우 aru 로 대치) */}
                    {
                        row.photo==null?
                            <img alt='' src={aru}
